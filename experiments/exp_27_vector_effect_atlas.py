"""
experiments/exp_27_vector_effect_atlas.py
=========================================
Systematically map how key safety-related vectors reshape model behavior.

This experiment is mechanism-first and non-leaking:
  - local risky prompts are used only as internal stimuli,
  - raw model continuations are not written to disk,
...
DEFAULT_CONDITIONS = [
    ...
    "soft_add",
    "late_safe_add",
    "exec_off_plus_soft_add",
    "exec_off_plus_late_safe_add",
]
    ...
    *,
    ...
    for topic, groups in sorted(topic_payload_train.items()):
        ...
        supportive_states = collect_hidden_states(
            ...
            groups["supportive"],
            layers=[LATE_SAFE_LAYER],
            desc=f"exp27.{topic}.support.train",
        )[LATE_SAFE_LAYER]
        ...
        topic_vectors[topic] = direction.to(r_exec.device)
...
def build_condition_hooks(
    *,
    ...
    if condition_name == "soft_ablate":
        return [(TARGET_LAYER, _make_ablate_hook(r_soft))]
    if condition_name == "soft_add":
        return [(TARGET_LAYER, _make_addition_hook(r_soft, alpha=soft_alpha))]
    if condition_name == "late_safe_add":
        return [(LATE_SAFE_LAYER, _make_addition_hook(late_safe_vec, alpha=late_safe_alpha))]
    if condition_name == "exec_off_plus_soft_add":
        return [
            (17, _make_ablate_hook(r_exec)),
            (23, _make_ablate_hook(r_exec)),
            (TARGET_LAYER, _make_addition_hook(r_soft, alpha=soft_alpha)),
        ]
    if condition_name == "exec_off_plus_late_safe_add":
        return [
            (17, _make_ablate_hook(r_exec)),
            (23, _make_ablate_hook(r_exec)),
...
def run_condition_for_rows(
    ...
    *,
    ...
    r_exec: torch.Tensor,
    r_soft: torch.Tensor,
    late_safe_by_topic: dict[str, torch.Tensor],
    soft_alpha: float,
    late_safe_alpha: float,
    max_new_tokens: int,
    ...
    for row in tqdm(rows, desc=f"exp27.{condition_name}"):
        late_safe_vec = late_safe_by_topic[row["topic"]]
        hook_specs = build_condition_hooks(
            ...
            late_safe_alpha=late_safe_alpha,
        )
        ...
        try:
            ...
            for layer in [TARGET_LAYER, LATE_SAFE_LAYER]:
        ...
        for layer in [TARGET_LAYER, LATE_SAFE_LAYER]:
            if layer not in holder:
                raise RuntimeError(
                    f"Failed to capture prompt state for layer {layer} under {condition_name}."
                )
            prompt_state_lists[layer].append(holder[layer])
...
def summarize_prompt_geometry(
    ...
    *,
    ...
    late_states = prompt_states[LATE_SAFE_LAYER]
    late_safe_stack = torch.stack(
        [late_safe_by_topic[row["topic"]].detach().float().cpu() for row in rows],
        dim=0,
    )
    out = {
        "z_exec_mean": float(projection_values_1d(target_states, r_exec).mean()),
        "z_soft_mean": float(projection_values_1d(target_states, r_soft).mean()),
        "z_late_safe_mean": float((late_states.float().cpu() * late_safe_stack).sum(dim=-1).mean().item()),
        "late_family_means": {},
    }
...
def summarize_audit_scores(rows: Sequence[dict[str, Any]], key: str) -> dict[str, Any]:
    ...
    return {
        "n": n,
        "mean": round(sum(values) / max(n, 1), 4),
...
def summarize_condition_records(
    ...
    *,
    ...
    for source_group in sorted({row["source_group"] for row in rows}):
        ...
        out["by_source_group"][source_group] = summarize_label_records(
            [row["label"] for row in group_rows]
        )
        harmful_group_rows = [
            row for row in group_rows
            if row["group"] == "harmful" and "audit_scores" in row
        ]
        if harmful_group_rows:
            out["by_source_group"][source_group]["harmful_risk"] = {
                "dangerous": summarize_audit_scores(harmful_group_rows, "dangerous"),
                DIRECT_DANGER_FEWSHOT_KEY: summarize_audit_scores(
                    harmful_group_rows,
                    DIRECT_DANGER_FEWSHOT_KEY,
                ),
            }
...
def build_effect_deltas(condition_summaries: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    baseline_actionable = baseline["label_summary"]["actionable_unsafe"]
    ...
    for condition_name, summary in condition_summaries.items():
        ...
        actionable = int(summary["label_summary"]["actionable_unsafe"])
        payload = {
            "delta_actionable_unsafe_rate": round(
                (actionable / max(int(summary["label_summary"]["n"]), 1))
            ...
            "delta_prompt_geometry": {
                ...
                for key in ["z_exec_mean", "z_soft_mean", "z_late_safe_mean"]
            },
        }
    ...
    *,
    ...
    items = [
        AuditItem(
            ...
            response=response_cache[idx],
            source_file="exp27_vector_effect_atlas",
    ...
    results = auditor.audit_items(
        ...
        progress=f"exp27.shield.{condition_name}",
    )
...
def main():
    ...
    parser.add_argument("--soft_alpha", type=float, default=4.0)
    ...
    parser.add_argument("--output", default="results/exp27_vector_effect_atlas.json")
    ...
    late_safe_by_topic = collect_topic_late_safe_vectors(
        model,
        tokenizer,
    ...
    per_condition_source_group_counts: dict[str, dict[str, dict[str, int]]] = {}
    ...
    for condition_name in condition_names:
        row_results, prompt_states = run_condition_for_rows(
            ...
            r_exec=r_exec,
            r_soft=r_soft,
            late_safe_by_topic=late_safe_by_topic,
        ...
        response_cache = {
            idx: row["response"]
            for idx, row in enumerate(row_results)
        }
        ...
        per_condition_source_group_counts[condition_name] = {
            source_group: summary["primary_counts"]
            for source_group, summary in condition_summaries[condition_name]["by_source_group"].items()
        }
    ...
    payload = {
        ...
        "direction_summary": {
            ...
            "r_soft_norm": float(r_soft.norm().item()),
            "cosine_r_exec_r_soft": float(torch.dot(r_exec, r_soft).item()),
            "late_safe_topic_norms": {
                topic: float(vec.norm().item())
                for topic, vec in sorted(late_safe_by_topic.items())
            },
            "cosine_r_exec_late_safe_by_topic": {
                topic: float(torch.dot(r_exec, vec).item())
                for topic, vec in sorted(late_safe_by_topic.items())
            },
            "cosine_r_soft_late_safe_by_topic": {
                topic: float(torch.dot(r_soft, vec).item())
                for topic, vec in sorted(late_safe_by_topic.items())
            },
        },
        "scope_release": scope_release,
        ...
        "source_group_primary_counts": per_condition_source_group_counts,
        "items": {
            condition_name: [
                {
                    "topic": row["topic"],
    ...
    print(f"[exp_27] saved={output_path}")
