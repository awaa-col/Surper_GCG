"""
experiments/exp_29_pure_detect_disentangle.py
=============================================
Disentangle the current detect candidate from a prompt-derived pure detect vector.

This experiment compares two vectors:
  1. detect_candidate: the current Exp19/28 detect-side candidate,
  2. detect_pure_prompt: harmful vs (supportive + safe_info) at prompt time, orthogonalized to r_exec.

Outputs are non-leaking:
...
DEFAULT_CONDITIONS = [
    ...
    "pure_add",
    "exec_off",
    "exec_off_plus_candidate_add",
    "exec_off_plus_pure_add",
]
...
def collect_pure_detect_prompt_vector(
    ...
    *,
    ...
    harmful_states = collect_hidden_states(
        ...
        harmful_prompts,
        layers=[TARGET_LAYER],
        desc="exp29.pure_detect.harm.train",
    )[TARGET_LAYER]
    safe_states = collect_hidden_states(
        ...
        safe_prompts,
        layers=[TARGET_LAYER],
        desc="exp29.pure_detect.safe.train",
    )[TARGET_LAYER]
...
def build_condition_hooks(
    *,
    ...
    if condition_name == "baseline":
        return []
    if condition_name == "candidate_off":
        return [(TARGET_LAYER, _make_ablate_hook(r_detect_candidate))]
    if condition_name == "pure_off":
        return [(TARGET_LAYER, _make_ablate_hook(r_detect_pure))]
    if condition_name == "candidate_add":
        return [(TARGET_LAYER, _make_addition_hook(r_detect_candidate, alpha=detect_alpha))]
    if condition_name == "pure_add":
    ...
    if condition_name == "exec_off_plus_candidate_add":
        return [
            (17, _make_ablate_hook(r_exec)),
            (23, _make_ablate_hook(r_exec)),
            (TARGET_LAYER, _make_addition_hook(r_detect_candidate, alpha=detect_alpha)),
        ]
    if condition_name == "exec_off_plus_pure_add":
        return [
            (17, _make_ablate_hook(r_exec)),
            (23, _make_ablate_hook(r_exec)),
            (TARGET_LAYER, _make_addition_hook(r_detect_pure, alpha=detect_alpha)),
        ]
...
def run_condition_for_rows(
    ...
    *,
    ...
    for row in tqdm(rows, desc=f"exp29.{condition_name}"):
...
def summarize_condition_records(rows: Sequence[dict[str, Any]], *, prompt_geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    for condition_name, summary in condition_summaries.items():
        ...
        payload = {
            ...
            "delta_prompt_geometry": {
                ...
                for key in ["z_exec_mean", "z_detect_candidate_mean", "z_detect_pure_mean"]
            },
        }
...
def attach_harmful_audit(*, rows: list[dict[str, Any]], auditor: ShieldGemmaAuditor, condition_name: str, truncate_response: int) -> None:
    ...
    items = [
        AuditItem(
            ...
            source_file="exp29_pure_detect_disentangle",
            source_path=condition_name,
            meta={"topic": rows[idx]["topic"], "group": rows[idx]["group"]},
        )
        for idx in harmful_indices
    ]
    results = auditor.audit_items(items, truncate_response=truncate_response, progress=f"exp29.shield.{condition_name}")
    for idx, result in zip(harmful_indices, results):
        rows[idx]["audit_scores"] = {key: float(score.yes_prob) for key, score in result.scores.items()}
...
def main():
    ...
    parser.add_argument("--output", default="results/exp29_pure_detect_disentangle.json")
    ...
    for condition_name in condition_names:
        ...
        condition_summaries[condition_name] = summarize_condition_records(row_results, prompt_geometry=prompt_geometry)
    ...
    print(f"[exp_29] saved={output_path}")
