"""
experiments/exp_38_whitebox_attack_feasibility.py
==================================================
...
Three analytical layers (per ChatGPT's recommendation):
  1. Is the gate truly open? → dangerous_mean, direct_danger_fewshot
  2. What does the open gate lead to? → actionable_unsafe_rate, label distribution,
                                        incoherent_rate (filter it out)
  3. Which intervention chain is most stable? → cross-topic consistency, delta tables
    ...
    *,
    ...
    items = [
        AuditItem(
            ...
            source_file="exp38_whitebox_attack_feasibility",
    ...
    results = auditor.audit_items(
        ...
        progress=f"exp38.shield.{condition}",
    )
...
def main():
    ...
    parser.add_argument("--output",
                        default="results/exp38_whitebox_attack_feasibility.json")
    ...
    # ── Model & direction ──────────────────────────────────────────────────────
    print("[exp38] Loading model...")
    model, tokenizer = load_model(args.model, args.hf_token)
    device = next(model.parameters()).device

    print("[exp38] Extracting r_exec direction...")
    ...
    print(f"[exp38] {len(eval_rows)} eval rows × {len(conditions)} conditions")
    ...
    # ── Three-layer verdict ────────────────────────────────────────────────────
    print("\n[exp38] Building attack feasibility verdict...")
    ...
    print("\n[exp38] === FEASIBILITY VERDICT SUMMARY ===")
    ...
    payload = {
        "experiment": "exp38_whitebox_attack_feasibility",
        "model": args.model,
        "seed": args.seed,
        "topics": topics,
        ...
        # All items (labels + scores, NO full response text here — see review_candidates)
        "items": {
            condition: [
                {
                    "topic": row["topic"],
